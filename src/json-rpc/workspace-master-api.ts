...
export interface IWorkspaceMasterApi {
  ...
  connect(): Promise<any>;
  subscribeEnvironmentOutput(workspaceId: string, callback: (text?: string) => void): void;
  unSubscribeEnvironmentOutput(workspaceId: string, callback: (text?: string) => void): void;
  subscribeWorkspaceStatus(workspaceId: string, callback: (text?: string) => void): void;
  unSubscribeWorkspaceStatus(workspaceId: string, callback: (text?: string) => void): void;
  subscribeOrganizationStatus(organizationId: string, callback: (text?: string) => void): void;
  unSubscribeOrganizationStatus(organizationId: string, callback: (text?: string) => void): void;
  subscribeOrganizationMembershipStatus(userId: string, callback: (text?: string) => void): void;
  unSubscribeOrganizationMembershipStatus(userId: string, callback: (text?: string) => void): void;
...
export class WorkspaceMasterApi implements IWorkspaceMasterApi {
  ...
  constructor(client: ICommunicationClient, entryPoint: string, refreshToken?: RefreshToken) {
  ...
  subscribeEnvironmentOutput(workspaceId: string, callback: IClientEventHandler): void {
    this.subscribe(
      MasterChannels.ENVIRONMENT_OUTPUT,
      MasterScopes.WORKSPACE,
      workspaceId,
      callback,
    );
  }
  ...
  subscribeWorkspaceStatus(workspaceId: string, callback: (text?: string) => void): void {
    ...
    this.subscribe(
      MasterChannels.WORKSPACE_STATUS,
      MasterScopes.WORKSPACE,
      workspaceId,
      statusHandler,
    );
  ...
  subscribeOrganizationStatus(organizationId: string, callback: IClientEventHandler): void {
    this.subscribe(
      MasterChannels.ORGANIZATION_STATUS,
      MasterScopes.ORGANIZATION,
      organizationId,
      callback,
    );
  }
  ...
  subscribeOrganizationMembershipStatus(userId: string, callback: IClientEventHandler): void {
    this.subscribe(
      MasterChannels.ORGANIZATION_MEMBERSHIP_STATUS,
      MasterScopes.USER,
      userId,
      callback,
    );
  ...
  private onConnectionClose(): void {
    ...
    setTimeout(() => {
      console.warn(
        `WebSocket is reconnecting, attempt #${this.reconnectionAttemptNumber} out of ${this.maxReconnectionAttempts}...`,
      );
      this.connect();
    }, delay);
  }
   ...
   */
  private subscribe(
    channel: MasterChannels,
    _scope: MasterScopes,
    id: string,
    callback: IClientEventHandler,
  ): void {
